...
from talus_web.file_upload import get_free_space
...
class DiskFree(APIView):
    def get(self, request):
        tmp_stat = os.statvfs('/tmp')
        free_space_in_tmp = tmp_stat.f_bsize * tmp_stat.f_bfree
        log.info('disk_free request. %d bytes free', free_space_in_tmp)
        response = Response(free_space_in_tmp)
        return response
...
class CodeCreate(APIView):
    ...
    def post(self, request, filename=None, format=None):
        ...
        return Response({"status": "success", "message": "git pull to see your new %s" % code_type})
